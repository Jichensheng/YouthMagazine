...
public class CheckView extends View {
	...
	private static final float SHADOW_WIDTH = 0; // dp
	...
	private void init(Context context) {
		...
		ace = getBitmapFromDrawable(R.drawable.ic_check_white_18dp, context);
	}
	...
	protected void onDraw(Canvas canvas) {
		super.onDraw(canvas);
		Paint transPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		transPaint.setStyle(Paint.Style.FILL);
		transPaint.setColor(Color.argb(50, 0, 0, 0));

		canvas.drawCircle((float) SIZE * mDensity / 2, (float) SIZE * mDensity / 2,
				BG_RADIUS * mDensity, transPaint);
		// draw white stroke
		canvas.drawCircle((float) SIZE * mDensity / 2, (float) SIZE * mDensity / 2,
		...
		if (ace != null) {
			...
			matrix.postScale(SIZE * mDensity * 0.5f/ace.getWidth(), SIZE * mDensity * 0.5f/ace.getHeight());
			matrix.postTranslate(SIZE * mDensity / 4, SIZE * mDensity / 4);
			canvas.drawBitmap(ace, matrix, mStrokePaint);
		}
		...
		if (mCountable) {
			...
			if (mChecked) {
				initBackgroundPaint();
				canvas.drawCircle((float) SIZE * mDensity / 2, (float) SIZE * mDensity / 2,
						(STROKE_RADIUS) * mDensity, mBackgroundPaint);
	...
	private void initBackgroundPaint() {
		if (mBackgroundPaint == null) {
		   ...
		   /* TypedArray ta = getContext().getTheme()
            ...
            int color = ta.getColor(0, defaultColor);
            ta.recycle();*/
			mBackgroundPaint.setColor(Color.rgb( 4, 171, 59));
		}
	}
