...
public class VirticleContentsView extends View {
	...
	private void drawChapter(Canvas canvas) {
		canvas.drawCircle(chapterRectSize / 2 + paddingChapter, chapterRectSize, chapterFontSizePx / 3, mPaint);
		...
		for (int i = 0; i < chapters.length; i++) {
			...
			if (i == chapters.length - 1 && titles.length != 0) {
				...
				canvas.drawLine(chapterRectSize / 2 + paddingChapter, chapterFontSizePx * 2, chapterRectSize / 2 + paddingChapter, getMeasuredHeight(), mPaint);
				mPaint.setStyle(Paint.Style.FILL);
			}
			canvas.restore();
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		//默认章节标题长度为4
		int height = ((chapterRectSize + paddingChapter) * 4 * 3) + chapterFontSize;
